...
const whitelistFileName = "./whitelistPhase2.csv"
...
function getProofs(rawData) {  
  ...
  for (const element of rawData) 
  {
    ...
    const price = element.split(",")[2].trim();
    const proof = ethers.utils.solidityKeccak256(['address', 'uint256', 'uint256'], [address, parseInt(qty), parseInt(price)])    
    result.push(proof);
  }
